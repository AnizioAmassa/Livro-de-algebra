...
\begin{document}
	...
	\section{Álgebra Vetorial}
		...
		\begin{definicao}
			...
			Se $V$ está no plano (\azul{$\mathbb{R}^2$} ou \azul{espaço euclidiano bidimensional}), então suas coordenadas são uma tupla de dois números reais, denotadas usualmente por $(v_1, v_2)$.
		...
		\begin{figure}[h]
			...
			\begin{subfigure}[t]{0.48\textwidth}
				...
				\begin{tikzpicture}[>=stealth,scale=1.2]
					...
					% Segmentos representando as diferenças
					\draw[thick,orange,yshift=-10pt]
					(1.2,-0.2) -- node[below] {\small $v_1$} (3.2,-0.2);
					
					\draw[thick,orange,xshift=-13pt]
					(-0.2,1.6) -- node[left] {\small $v_2$} (-0.2,2.4);
		...
		\begin{figure}[h]
			...
			\end{tikzpicture}
			\caption{Norma de um vetor $ V$ em $\mathbb{R}^2$.}
		\end{figure}
		...
		\begin{figure}[h]
			...
			\begin{tikzpicture}[>=stealth,scale=1.2]
				...
				\draw[dashed] (V) -- (P)
				node[midway,right] {$ V - \operatorname{proj}_W V$};
				
				% Marcação do ângulo reto
				\tkzMarkRightAngle(W,P,V)
				
			\end{tikzpicture}
			\caption{Projeção ortogonal de $ V$ sobre $ W$.}
		\end{figure}
		...
		\end{figure}
		Como sabemos, a área do paralelogramo é dada pelo produto da base com a altura, enquanto o volume do paralelepípedo é dado pelo produto da área da base com a altura. Assim seguem as relações abaixo.
		\[A_{VW}=bh=\|V\|h=\|V\|\|W\|\sin(\theta_1)=\|V\times W\|\]
		\[V_{VWU}=A_{VW}h=A_{VW}\|U\|\cos(\theta_2)=\|V\times W\|\|U\|\cos(\theta_2)=|[V,W,U]|\]
		...
		\begin{proposicao}
			...
			\begin{itemize}
				\item $V$ e $W$ são colineares $\Leftrightarrow \exists \, \alpha \in \mathbb{R}\text{, tal que } V=\alpha W \text{ ou } W=\alpha V$, ou seja, se um for múltiplo escalar do outro;
		...
		\begin{corolario}
			...
			\end{enumerate}
		\end{corolario}
		Um fato interessante é que, a partir do item \textit{i)} do corolário acima, obtemos que a equação $xV+yW+zU=\bar{0}$ tem solução onde $x^2+y^2+z^2\neq 0$, ou seja, em que pelo menos um dos valores é diferente de zero. Isso porque, se existem reais $\alpha, \beta$ tais que $\alpha V+\beta W= U$ (definição de combinação linear), então basta tomar $x=\alpha$, $y=\beta$ e $z=-1$.
	...
	\section{Geometria Analítica}
		...
		\subsection*{Equações da reta e do plano}
			\begin{definicao}
				...
				\]
				onde $P_0$ é um ponto fixo e $V\in \mathbb{R}^2$ (ou $\mathbb{R}^3$) é o vetor diretor.
			\end{definicao}
			\begin{definicao}
		...
		\subsection*{Ângulos e distâncias}
			...
			\begin{proposicao}
				...
				\end{cases}
				\]
				O cosseno do ângulo entre $\pi_1$ e $\pi_2$ é
				\[
			...
			\begin{proposicao}
				...
				\]
				em que $V=(a,b,c)$ é um vetor diretor e $P_1=(x_1,y_1,z_1)$ é um ponto da reta $r$.
			\end{proposicao}
