...
\begin{document}
	...
	\smallskip
	\begin{teorema}
		Sejam $v_1,...,v_m$ vetores não nulos do espaço vetorial $E$. Se nenhum deles é combinação linear dos anteriores então o conjunto $X={v_1,...,v_m}$ é L.I.
	\end{teorema}
	
	\textbf{Prova:} Suponha por absurdo que uma combinação linear dos vetores dados, com coeficientes não todos nulos, fosse igual a zero. Se $a_r v_r$ fosse a última parcela não nula dessa combinação, teríamos então
	$$a_1 v_1 + ... + a_r v_r = 0$$
	com $a_r \neq 0$. Daí viria $v_r = -\frac{a_1}{a_r}v_1 - ... - \frac{a_{r_1}}{a_r}v_1$, logo $v_r$ seria uma combinação linear dos outros, ocasionando em uma absurdo. \qed
	\smallskip
