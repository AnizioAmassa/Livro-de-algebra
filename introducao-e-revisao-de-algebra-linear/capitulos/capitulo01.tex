...
\begin{document}
	...
	\section{Álgebra Vetorial}
		...
		Essas definições permitem o entendimento de que a norma do produto vetorial é a área do paralelogramo formado pelos vetores $V$ e $W$, e o módulo do produto misto é o volume do paralelepípedo.
		...
		\end{figure}
		Como sabemos, a área do paralelogramo é dada pelo produto da base com a altura, enquanto o volume do paralelepípedo é dado pelo produto da área da base com a altura. Assim seguem as relações abaixo. \[A_{VW}=\|V\|\|W\|\sin(\theta_1)=\|V\times W\|\] \[V_{VWU}=A_{VW}\|U\|\cos(\theta_2)=\|V\times W\|\|U\|\cos(\theta_2)=|[V,W,U]|\]
		...
		\begin{corolario}
			...
			\end{enumerate}
		\end{corolario}
		Um fato interessante é que, a partir do item \textit{i)} do corolário acima, obtemos que a equação $xV+yW+zU=\bar{0}$ tem solução não trivial, ou seja, tem solução onde $x,y,z\neq 0$. Isso porque, se existem reais $\alpha, \beta$ tais que $\alpha V+\beta W= U$ (definição de combinação linear), então basta tomar $x=\alpha$, $y=\beta$ e $z=-1$.
		
		Com isso, finalizamos a parte de Álgebra Vetorial. Seguem abaixo os exercícios da seção.\\ \bigskip
	...
	\section{Geometria Analítica}
		...
		\begin{definicao}
			Uma reta $r \subset \mathbb{R}^2$ (ou $\mathbb{R}^3$) é o conjunto dos pontos $P$
			dados por
			\[
		...
		\end{definicao}
		\begin{definicao}
			Um plano $\pi \subset \mathbb{R}^3$ é o conjunto dos pontos $P$ dados por
			\[
		...
		\begin{figure}[h]
			...
			\begin{subfigure}[t]{0.45\textwidth}
				...
				\begin{tikzpicture}[>=stealth,scale=1.2]
					...
					\fill (P) circle (1pt) node[below right] {$P=P_0+t\vec v$};
				...
				\end{tikzpicture}
				\caption{Reta $r \subset \mathbb{R}^2$ dada por $P=P_0+t\vec v$.}
			\end{subfigure}
			\hfill
			% ===================== (b) Plano em R^3 =====================
			\begin{subfigure}[t]{0.45\textwidth}
				...
				\begin{tikzpicture}[tdplot_main_coords,scale=2.2,>=stealth]
					...
					% eixo z maior, para pegar todo o plano
					\draw[->] (0,0,-0.5) -- (0,0,4.0) node[above] {$z$};
					
					% ponto P0 e vetores diretores (AGORA mais inclinados em z)
					\coordinate (P0)  at (0.6,0.5,0.3);
					
					% P0 + v  (v = (1.4,0.2,1.1))
					\coordinate (V)   at (2.0,0.7,1.4);
					
					% P0 + w  (w = (0.4,1.3,1.2))
					\coordinate (W)   at (1.0,1.8,1.5);
					
					% P0 + v + w
					\coordinate (VW)  at (2.4,2.0,2.6);
					
					%----------------------------------------------------
					% 1) Grande "faixa" do plano (retângulo maior no mesmo plano)
					%    P0 + a v + b w
					%----------------------------------------------------
					\coordinate (A) at (0.06,0.05,-0.39);  % ~ P0 - 0.3 v - 0.3 w
					\coordinate (B) at (2.58,0.41,1.59);   % ~ P0 + 1.5 v - 0.3 w
					\coordinate (C) at (3.30,2.75,3.75);   % ~ P0 + 1.5 v + 1.5 w
					\coordinate (D) at (0.78,2.39,1.77);   % ~ P0 - 0.3 v + 1.5 w
					...
					node[pos=0.55,below right] {$\vec v$};
					\draw[->,thick,red] (P0) -- (W)
					node[pos=0.55,above left] {$\vec w$};
					
					% ponto P no plano (P = P0 + t v + s w, por ex. t=1.1, s=0.6)
					\coordinate (P) at (2.38,1.5,2.23);
					\fill (P) circle (0.8pt);
					\node[above right] at (P) {$P=P_0+t\vec v+s\vec w$};
					
					% ponto P0
					\fill (P0) circle (0.9pt);
					\node[left] at (P0) {$P_0$};
					
					% nome do plano (no meio do paralelogramo)
				...
				\caption{Plano $\pi \subset \mathbb{R}^3$ dado por $P=P_0+t\vec v+s\vec w$.}
			\end{subfigure}
			
			\caption{Representação paramétrica de reta e plano por pontos e vetores diretores.}
		\end{figure}
